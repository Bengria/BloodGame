...
{ 
    ...
    private void Update()
    {
        // Проверяем ввод для прыжка (например, пробел)
        if (Input.GetKeyDown(KeyCode.Space) && !isJumping)
        {
            Jump();
        }
        // Проверяем ввод для передвижения (например, клавиши WASD)
    ...
    private void Jump()
    {
        ...
        // Здесь можно добавить логику для физики прыжка
    }
    private void Move()
    {
        isMoving = true;
        animator.SetBool("IsMoving", true);
        // Здесь можно добавить логику для передвижения персонажа (например, перемещение через Translate)
        // Для примера, используем Translate для передвижения вперед
    ...
    // Метод, вызываемый из анимации для сигнала окончания прыжка
